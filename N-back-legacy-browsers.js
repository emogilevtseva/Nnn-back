...
flowScheduler.add(endd_2RoutineBegin());
flowScheduler.add(endd_2RoutineEachFrame());
flowScheduler.add(endd_2RoutineEnd());
...
async function experimentInit() {
  ...
  instructions = new visual.TextStim({
    win: psychoJS.window,
    name: 'instructions',
    text: 'Добрый день! это тест N-back.\n\nВ этом задании вам нужно нажимать пробел, если белый квадрат появляется в том же месте, что и в предыдущем испытании.\nНапример, если квадрат был в левом нижнем углу в первом испытании и затем появился в том же месте во втором, нажмите пробел.\nВ противном случае ничего не делайте.\n\n\nНажмите пробел, чтобы продолжить.',
    font: 'Arial',
    units: undefined, 
  ...
  instructions_2 = new visual.TextStim({
    win: psychoJS.window,
    name: 'instructions_2',
    text: 'Это конец серии испытаний N-back-1.\nСейчас вы начнёте серию N-back-2.\n\n\nЭто означает, что теперь нужно нажимать пробел не тогда, когда квадрат появляется в той же позиции, что и в предыдущем испытании, а тогда, когда он появляется в той же позиции, что и два испытания назад.\n\nНапример, если квадрат появился в левом нижнем углу в первом испытании, вам нужно нажать пробел, если он снова появится в левом нижнем углу в третьем испытании.\n\n\n\nНажмите пробел, чтобы продолжить.',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0], draggable: false, height: 0.05,  wrapWidth: undefined, ori: 0,
    languageStyle: 'LTR',
  ...
  // Initialize components for Routine "endd_2"
  endd_2Clock = new util.Clock();
...
var endd_2MaxDurationReached;
var endd_2MaxDuration;
var endd_2Components;
function endd_2RoutineBegin(snapshot) {
      return async function () {
        TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
        
        //--- Prepare to start Routine 'endd_2' ---
        t = 0;
        frameN = -1;
        continueRoutine = true; // until we're told otherwise
        // keep track of whether this Routine was forcibly ended
        routineForceEnded = false;
        endd_2Clock.reset(routineTimer.getTime());
        routineTimer.add(1.000000);
        endd_2MaxDurationReached = false;
        // update component parameters for each repeat
        ...
        body: JSON.stringify({ 
        experimentID: 'rRi54E5Toi4Z', // * обновить, указав experiment ID из DATAPIPE на шаге 4.3 * 
        filename: filename,  
        data: data, 
        }), 
        ...
        psychoJS.experiment.addData('endd_2.started', globalClock.getTime());
        endd_2MaxDuration = null
        // keep track of which components have finished
        endd_2Components = [];
        endd_2Components.push(text_2);
        
        endd_2Components.forEach( function(thisComponent) {
          if ('status' in thisComponent)
            thisComponent.status = PsychoJS.Status.NOT_STARTED;
           });
        return Scheduler.Event.NEXT;
...
function endd_2RoutineEachFrame() {
      return async function () {
        //--- Loop for each frame of Routine 'endd_2' ---
        // get current time
        t = endd_2Clock.getTime();
        frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
        // update/draw components on each frame
...
function endd_2RoutineEnd(snapshot) {
      return async function () {
        //--- Ending Routine 'endd_2' ---
        endd_2Components.forEach( function(thisComponent) {
          if (typeof thisComponent.setAutoDraw === 'function') {
            thisComponent.setAutoDraw(false);
          }
        });
        psychoJS.experiment.addData('endd_2.stopped', globalClock.getTime());
        if (routineForceEnded) {
            routineTimer.reset();} else if (endd_2MaxDurationReached) {
            endd_2Clock.add(endd_2MaxDuration);
        } else {
            endd_2Clock.add(1.000000);
        }
        // Routines running outside a loop should always advance the datafile row
        if (currentLoop === psychoJS.experiment) {
          psychoJS.experiment.nextEntry(snapshot);
